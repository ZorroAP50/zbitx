...
void key_draw(struct field *f){
	...
	char display_label[8]; // wyświetlany tekst klawisza
	...
	else if (!strcmp(f->label, "Sym")){
		if (edit_state == EDIT_STATE_SYM)
			background_color = TFT_ORANGE;
	}
	else if (!strcmp(f->label, "ABC")){
		// podświetl i zmień napis zależnie od stanu
		if (edit_state == EDIT_STATE_UPPER)
			background_color = TFT_CYAN;
		// napis "ABC" lub "abc" obsłużymy niżej
	}

	// ustal co wyświetlić na klawiszu
	bool is_letter = (strlen(f->label) == 1 && isalpha(f->label[0]));
	...
	if (edit_state == EDIT_STATE_SYM){
		...
		// pokaż "ABC" gdy duże, "abc" gdy małe
		const char *abc_label = (edit_state == EDIT_STATE_UPPER) ? "ABC" : "abc";
		x -= measure_text(abc_label, ZBITX_FONT_LARGE)/2;
	} else if (is_letter && edit_state == EDIT_STATE_ALPHA){
		// małe litery — oblicz szerokość małej litery
		display_label[0] = tolower(f->label[0]);
		display_label[1] = 0;
		x -= measure_text(display_label, ZBITX_FONT_LARGE)/2;
	} else {
	...
	if (edit_state == EDIT_STATE_SYM){
		...
		const char *abc_label = (edit_state == EDIT_STATE_UPPER) ? "ABC" : "abc";
		screen_draw_text(abc_label, -1, x, (f->y)+9, text_color, ZBITX_FONT_LARGE);
	} else if (is_letter && edit_state == EDIT_STATE_ALPHA){
		display_label[0] = tolower(f->label[0]);
		display_label[1] = 0;
		screen_draw_text(display_label, -1, x, (f->y)+9, text_color, ZBITX_FONT_LARGE);
	} else {
		screen_draw_text(f->label, -1, x, (f->y)+9, text_color, ZBITX_FONT_LARGE);
	}
}
...
char keyboard_read(struct field *key){
  ...
  if (!strcmp(key->label, "space"))
    c = ' ';
	else if (!strcmp(key->label, "ABC")){
		// przełącznik wielkie/małe litery
		if (edit_state == EDIT_STATE_UPPER){
			edit_state = EDIT_STATE_ALPHA;
			edit_mode  = EDIT_STATE_ALPHA;
		} else {
			edit_state = EDIT_STATE_UPPER;
			edit_mode  = EDIT_STATE_UPPER;
		}
		keyboard_redraw();
		return 0;
	}
...
void text_draw(struct field *f){
	...
	// ukryte hasło — rysuj gwiazdki zamiast znaków
	char masked[FIELD_TEXT_MAX_LENGTH];
	if (!strcmp(f->label, "WIFI_PASS")){
		struct field *f_show = field_get("WIFI_SHOWPASS");
		bool hide = !(f_show && !strcmp(f_show->value, "1"));
		if (hide){
			int len = strlen(f->value);
			for (int i = 0; i < len && i < FIELD_TEXT_MAX_LENGTH-1; i++)
				masked[i] = '*';
			masked[len] = 0;
			p = masked;
		}
