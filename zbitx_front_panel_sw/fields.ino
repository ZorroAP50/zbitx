...
struct field *dialog_box(const char *title, char const *fields_list){
	...
	struct field *f_mode_field = field_get("MODE");
	if (!f_mode_field) return NULL;
	strcpy(original_mode, f_mode_field->value);
...
void field_set(const char *label, const char *value, bool update_to_radio){
  ...
  else if (!strcmp(f->label, "WIFI_LIST")){
    wifi_list_update(value);
    f->redraw = true;
  }
  else if (!strcmp(f->label, "WIFI_IP")){
...
struct field *field_select(const char *label){
  ...
  if (!f){
	...
	// lista WiFi - obsługa dotyku bezpośrednio
	if (f->type == FIELD_WIFI_LIST){
		// potrzebujemy współrzędnych dotyku - pobieramy z ekranu
		uint16_t tx, ty;
		if (screen_read(&tx, &ty))
			wifi_list_select(f, tx, ty);
		return NULL;
	}

	if (!strcmp(f->label, "MENU")){
		dialog_box("Radio", "10M/12M/15M/17M/20M/30M/40M/60M/80M/AGC/VFO/SPLIT/WIFI/CLOSE");
		return NULL;
	}
	...
	if (!strcmp(f->label, "WIFI")){
		// wyczyść listę przed otwarciem
		wifi_list_init();
		struct field *fw = field_get("WIFI_LIST");
		if (fw){ fw->value[0] = 0; fw->redraw = true; }
		// zaktualizuj IP przed otwarciem dialogu
		struct field *fip = field_get("WIFI_IP");
		if (fip) fip->redraw = true;
		dialog_box("WiFi", "WIFI_IP/WF_SSID_LBL/WIFI_SSID/WIFI_SCAN/WF_PASS_LBL/WIFI_PASS/WIFI_SHOWPASS/WIFI_SAVE/WIFI_OFF/WIFI_CLOSE/WIFI_LIST/WIFI_STATUS");
		return NULL;
	}
	...
	if (!strcmp(f->label, "WIFI_SAVE")){
		struct field *f_ssid = field_get("WIFI_SSID");
		struct field *f_pass = field_get("WIFI_PASS");
		if (f_ssid) field_post_to_radio(f_ssid);
		if (f_pass) field_post_to_radio(f_pass);
		field_post_to_radio(f);
		field_set("WIFI_STATUS", "Connecting...", false);
		return NULL;
	}

	if (!strcmp(f->label, "WIFI_SCAN")){
		field_set("WIFI_STATUS", "Scanning...", false);
		field_set("WIFI_LIST", "", false);
		wifi_list_init();
		field_post_to_radio(f);  // wyślij "WIFI_SCAN" do Pi Zero
		return NULL;
	}

	if (!strcmp(f->label, "WIFI_OFF")){
		field_post_to_radio(f);
		field_set("WIFI_STATUS", "WiFi off", false);
		return NULL;
	}

	// WIFI_SHOWPASS - FIELD_SELECTION przełączy wartość sam dalej w kodzie
	// tutaj tylko zaznaczamy że hasło wymaga przerysowania
	if (!strcmp(f->label, "WIFI_SHOWPASS")){
		struct field *f_pass = field_get("WIFI_PASS");
		if (f_pass) f_pass->redraw = true;
		// nie zwracamy NULL - pozwalamy na normalne przetworzenie FIELD_SELECTION
	}
	...
	else if (f_selected && !strcmp(f_selected->label, "SAVE")){
		logbook_init();
	}
...
void smeter_draw(struct field *f, bool force){
	char temp_str[100];
	static int last_s = -1;
	static int last_in_tx = -1;
	static int last_vfwd = -1;
	static int last_vswr = -1;
	static int last_vbatt = -1;

	struct field *f_tx = field_get("IN_TX");
	int cur_in_tx = f_tx ? atoi(f_tx->value) : 0;
	int cur_s     = atoi(field_get("SMETER")->value) / 200;
	int cur_vfwd  = vfwd / 10;
	int cur_vswr  = vswr;
	int cur_vbatt = vbatt / 10;

	bool changed = (cur_in_tx != last_in_tx || cur_s != last_s ||
	                cur_vfwd  != last_vfwd  || cur_vswr != last_vswr ||
	                cur_vbatt != last_vbatt);

	if (!force && !changed)
		return;

	bool tx_just_ended = (last_in_tx != 0 && cur_in_tx == 0);

	last_in_tx = cur_in_tx;
	last_s     = cur_s;
	last_vfwd  = cur_vfwd;
	last_vswr  = cur_vswr;
	last_vbatt = cur_vbatt;

	// napięcie - od x+150, mieści się w 189px pola
	screen_fill_rect(f->x + 150, f->y, 39, f->h, TFT_BLACK);
	int v = vbatt / 10;
	sprintf(temp_str, "+%d.%dv", v/10, v%10);
	screen_draw_text(temp_str, -1, f->x + 151, f->y+1, TFT_WHITE, 1);

	if (!f_tx)
		return;

	if (cur_in_tx){
		screen_fill_rect(f->x, f->y, 149, f->h, TFT_BLACK);
		// moc po lewej
		sprintf(temp_str, "%dW", vfwd/10);
		screen_draw_text(temp_str, -1, f->x + 2, f->y + 1, TFT_WHITE, 2);
		// pasek mocy - ramka i wypełnienie
		int bar = (vfwd > 55) ? 55 : vfwd;
		screen_draw_rect(f->x + 48,  f->y + 2, 57, 12, TFT_YELLOW);
		screen_fill_rect(f->x + 49,  f->y + 3, bar, 10, TFT_RED);
		// SWR po prawej
		sprintf(temp_str, "SWR%d.%d", vswr/10, vswr%10);
		screen_draw_text(temp_str, -1, f->x + 108, f->y + 1, TFT_WHITE, 2);
		return;
	}

	// tryb RX - 8 segmentów × 17px = 136px + 3px offset = 139px, miesci sie przed vbatt
	// po zmianie TX→RX lub force wyczyść obszar segmentów
	if (tx_just_ended || force)
		screen_fill_rect(f->x, f->y, 149, f->h, TFT_BLACK);
	...
	for (int i = 0; i < 8; i++){
		...
		if (cur_s > i){
			switch(i){
				case 0: case 1:           color = TFT_YELLOW; break;
				case 2: case 3: case 4:   color = TFT_GREEN;  break;
				case 5:                   color = TFT_ORANGE; break;
				case 6: case 7:           color = TFT_RED;    break;
			}
		}
		int sx = f->x + 3 + (i * 17);
		screen_fill_rect(sx, f->y + 1, 14, 5, color);

		if (i == 0)      strcpy(temp_str, "S1");
		else if (i == 1) strcpy(temp_str, "S3");
		else if (i == 2) strcpy(temp_str, "S5");
		else if (i == 3) strcpy(temp_str, "S7");
		else if (i == 4) strcpy(temp_str, "S9");
		else if (i == 5) strcpy(temp_str, "+10");
		else if (i == 6) strcpy(temp_str, "+20");
		else             strcpy(temp_str, "+40");
		screen_draw_text(temp_str, -1, sx, f->y + 7, TFT_WHITE, 1);
	}
}
...
void field_static_draw(field *f){
	...
	// jeśli pole ma wysokość (nie jest etykietą bez tła), wyczyść przed rysowaniem
	if (f->h > 0)
...
void field_draw(struct field *f){
	...
	if (f->type != FIELD_WATERFALL && f->type != FIELD_FT8 && f->type != FIELD_LOGBOOK && f->type != FIELD_KEY
		...
		f->type != FIELD_CONSOLE && f->type != FIELD_SMETER && f->type != FIELD_WIFI_LIST){
    //skip the background fill for smeter, console, waterfall etc.
  ...
  // checkbox WIFI_SHOWPASS - rysuj niezależnie od typu pola
  if (!strcmp(f->label, "WIFI_SHOWPASS")){
    bool checked = !strcmp(f->value, "1");
    screen_fill_round_rect(f->x+2, f->y+2, f->w-4, f->h-4, checked ? TFT_DARKGREY : TFT_BLACK);
    screen_draw_rect(f->x+6, f->y + f->h/2 - 7, 14, 14, TFT_WHITE);
    if (checked)
      screen_fill_rect(f->x+9, f->y + f->h/2 - 4, 8, 8, TFT_GREEN);
    screen_draw_text("Show password", -1, f->x+26, f->y+f->h/2-7, TFT_WHITE, ZBITX_FONT_NORMAL);
    return;
  }
  ...
  for (f = field_list; f->type != -1; f++)
    if ((all || f->redraw) && f->is_visible){
			if (edit_mode == -1 || f->y + f->h < 144 || f->type == FIELD_KEY
				|| f_selected == f){
				if (f->type == FIELD_FREQ)
					freq_redrawn = true;
      	field_draw(f);
      	f->redraw = false;
			}
    }
	// METERS leży wewnątrz obszaru FREQ — musi być rysowany po FREQ
	// force=true gdy FREQ było przerysowane (jego fill_round_rect zamazało METERS)
	f = field_get("METERS");
	if (f) smeter_draw(f, all || freq_redrawn);
}
